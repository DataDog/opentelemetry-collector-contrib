...
func NewServer(
	...
	srv := &Server{
		...
		collectorConfigStringMap: collectorconfigstringmap,
...
func (s *Server) PrepareAndSendFleetAutomationPayloads() (*payload.OtelCollector, error) {
	logger := s.logger
	collectorConfigStringMap := *s.collectorConfigStringMap
	otelCollectorPayload := *s.otelCollectorPayload

	moduleInfoJSON := componentchecker.PopulateFullComponentsJSON(*s.moduleInfo, collectorConfigStringMap)
	activeComponents, err := componentchecker.PopulateActiveComponents(collectorConfigStringMap, moduleInfoJSON, *s.componentStatus, logger)
	if err != nil {
	...
	healthStatus := componentchecker.DataToFlattenedJSONString(*s.componentStatus, true)
