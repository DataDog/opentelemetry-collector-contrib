...
import (
	...
	"github.com/DataDog/opentelemetry-collector-contrib/e2etests/otelcollector/helm"
...
func Provisioner(opts ...ProvisionerOption) provisioners.TypedProvisioner[Kubernetes] {
	// We ALWAYS need to make a deep copy of `params`, as the provisioner can be called multiple times.
	// and it's easy to forget about it, leading to hard to debug issues.
	params := newProvisionerParams()
	_ = optional.ApplyOptions(params, opts)

	provisioner := provisioners.NewTypedPulumiProvisioner(provisionerBaseID+params.name, func(ctx *pulumi.Context, env *Kubernetes) error {
		// We ALWAYS need to make a deep copy of `params`, as the provisioner can be called multiple times.
		// and it's easy to forget about it, leading to hard to debug issues.
		params := newProvisionerParams()
		_ = optional.ApplyOptions(params, opts)
...
func RunFunc(ctx *pulumi.Context, env *Kubernetes, params *ProvisionerParams) error {
	...
	awsCommonEnv := awsEnv.GetCommonEnvironment()
	...
	kindCluster, err := kubeComp.NewKindCluster(&awsEnv, host, params.name, awsCommonEnv.KubernetesVersion(), utils.PulumiDependsOn(installEcrCredsHelperCmd))
	if err != nil {
		return err
	}
